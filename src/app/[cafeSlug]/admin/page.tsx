...
const TAB_TO_HASH: Record<string, string> = {
  products: "menu",
  modifiers: "modifiers", // 🌟 تمت إضافة الرابط
  qr: "tables",
  sales: "sales",
  devices: "devices",
  employees: "employees",
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-muted/20 p-6 md:p-12 font-sans" dir={dir}>
        <header className="mb-8 flex flex-col xl:flex-row xl:items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-border gap-4 relative z-50">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex flex-wrap bg-muted p-1 rounded-xl gap-1">
              ...
              {/* 🌟 زر الإضافات الجديد المضاف هنا */}
              <button onClick={() => switchTab('modifiers')} className={`flex items-center gap-2 px-5 py-3 rounded-lg font-bold text-sm ${activeTab === 'modifiers' ? 'bg-white text-primary shadow-sm' : 'text-muted-foreground'}`}><Wand2 size={18} /> {t.tabModifiers}</button>
        ...
        {/* 🌟 عرض مكون الإضافات المضاف هنا */}
        {activeTab === 'modifiers' && <ModifiersTab cafeId={cafeId!} activeLang={activeLang} />}
